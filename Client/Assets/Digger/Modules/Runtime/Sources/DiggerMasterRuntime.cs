...
namespace Digger.Modules.Runtime.Sources
{
    ...
    public class DiggerMasterRuntime : ADiggerRuntimeMonoBehaviour
    {
        ...
        {
            var existingDiggerSystem = FindAnyObjectByType<DiggerSystem>();
        ...
        {
            var allSystems = FindObjectsByType<DiggerSystem>(FindObjectsSortMode.None);
            foreach (var diggerSystem in allSystems)
            {
                Init(diggerSystem);
            }

            diggerSystems = allSystems;
        }
