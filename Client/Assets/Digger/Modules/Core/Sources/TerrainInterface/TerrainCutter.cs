...
namespace Digger.Modules.Core.Sources.TerrainInterface
{
    ...
    public class TerrainCutter : MonoBehaviour
    {
        ...
        private UndoRecord buildState;

        /// <summary>
        /// Saves the current terrain state (holes, details, trees) as the build-time state.
        /// Must be called after Refresh() and before loading any runtime-persisted data.
        /// </summary>
        public void SaveBuildState()
        {
            buildState = CreateUndoRecord();
        }

        /// <summary>
        /// Restores the terrain to the build-time state that was saved by SaveBuildState().
        /// </summary>
        public void RestoreBuildState()
        {
            if (buildState == null)
                return;

            RestoreUndoRecord(buildState);
        }
