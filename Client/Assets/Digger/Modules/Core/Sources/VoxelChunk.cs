...
namespace Digger.Modules.Core.Sources
{
    public class VoxelChunk : MonoBehaviour
    {
        ...
        [NonSerialized] private UnityEngine.EntityId needToBakePhysicMeshEntityId;
#else
        ...
        {
#if UNITY_6000_1_OR_NEWER
            needToBakePhysicMeshEntityId = GetPolygonizer(lod).CompleteBuildMesh(mesh, Digger.GetChunkBounds()) ?
                mesh.GetEntityId() : default;
#else
        ...
        {
#if UNITY_6000_1_OR_NEWER
            if (needToBakePhysicMeshEntityId == default)
                return;
            var job = new PhysicsBakeMeshJob
            {
                MeshEntityId = needToBakePhysicMeshEntityId
            };
#else
